...
sort__dcacheline_cmp(struct hist_entry *left, struct hist_entry *right)
{
    ...
    if (l_map->ino_generation > r_map->ino_generation) {
        return -1;
    }
    if (l_map->ino_generation < r_map->ino_generation) {
        return 1;
    ...
    if ((left->cpumode != PERF_RECORD_MISC_KERNEL) &&
        ...
        if (left->thread->pid_ > right->thread->pid_) {
            return -1;
        }
        if (left->thread->pid_ < right->thread->pid_) {
            return 1;
