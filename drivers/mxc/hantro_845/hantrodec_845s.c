...
#define DRIVER_NAME "mxc_hantro_845"
...
static void read_core_cfg(hantrodec_t *dev)
{
	int c = dev->core_id;
	u32 reg, tmp, mask;
...
static int hantrodec_init(struct platform_device *pdev, int id)
{
	...
	read_core_cfg(&hantrodec_data[id]);
...
static int __exit hantro_dev_remove(struct platform_device *pdev)
{
...
static struct platform_driver mxchantro_driver = {
	...
	.remove = __exit_p(hantro_dev_remove),
};
module_platform_driver_probe(mxchantro_driver, hantro_dev_probe);

MODULE_LICENSE("GPL v2");
MODULE_DESCRIPTION("Hantro VPU Decoder driver");
