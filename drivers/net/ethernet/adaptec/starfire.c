...
static int netdev_open(struct net_device *dev)
{
    ...
    for (i = 1; i < 16; i++) {
        ...
        writew(be16_to_cpu(eaddrs[2]), setup_frm);
        setup_frm += 4;
        writew(be16_to_cpu(eaddrs[1]), setup_frm);
        setup_frm += 4;
        writew(be16_to_cpu(eaddrs[0]), setup_frm);
        setup_frm += 8;
    }
...
static void set_rx_mode(struct net_device *dev)
{
    ...
    if (dev->flags & IFF_PROMISC) {	/* Set promiscuous. */
        ...
        netdev_for_each_mc_addr(ha, dev) {
            ...
            writew(be16_to_cpu(eaddrs[2]), filter_addr);
            filter_addr += 4;
            writew(be16_to_cpu(eaddrs[1]), filter_addr);
            filter_addr += 4;
            writew(be16_to_cpu(eaddrs[0]), filter_addr);
            filter_addr += 8;
        }
