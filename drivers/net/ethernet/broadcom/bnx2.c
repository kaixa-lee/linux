...
bnx2_set_coalesce(struct net_device *dev, struct ethtool_coalesce *coal)
{
    ...
    if (bp->rx_ticks > 0x3ff) {
        bp->rx_ticks = 0x3ff;
    }

    bp->rx_quick_cons_trip = (u16) coal->rx_max_coalesced_frames;
    if (bp->rx_quick_cons_trip > 0xff) {
        bp->rx_quick_cons_trip = 0xff;
    }

    bp->rx_ticks_int = (u16) coal->rx_coalesce_usecs_irq;
    if (bp->rx_ticks_int > 0x3ff) {
        bp->rx_ticks_int = 0x3ff;
    }
    ...
    if (bp->tx_ticks > 0x3ff) {
        bp->tx_ticks = 0x3ff;
    }

    bp->tx_quick_cons_trip = (u16) coal->tx_max_coalesced_frames;
    if (bp->tx_quick_cons_trip > 0xff) {
        bp->tx_quick_cons_trip = 0xff;
    }

    bp->tx_ticks_int = (u16) coal->tx_coalesce_usecs_irq;
    if (bp->tx_ticks_int > 0x3ff) {
        bp->tx_ticks_int = 0x3ff;
    }
