...
static void handlefc(struct net_device *dev)
{
    ...
    if ( wait_timeout(dev,0xfc) ) {
        printk("timed out in handlefc\n");
    }
}
...
static void handlefd(struct net_device *dev)
{
    ...
    if ( wait_timeout(dev,0xfd) ) {
        printk("timed out in handlefd\n");
    }
    sendup_buffer(dev);
}
...
static void handleread(struct net_device *dev)
{
    ...
    inb_p(base+2);
    if ( wait_timeout(dev,0xfb) ) {
        printk("timed out in handleread\n");
    }
}
...
static void handlecommand(struct net_device *dev)
{
    ...
    inb_p(base+3);
    inb_p(base+2);
    if ( wait_timeout(dev,0xfa) ) {
        printk("timed out in handlecommand\n");
    }
}
...
static int __init ltpc_probe_dma(int base, int dma)
{
    ...
    ltdmabuf[2] = 0;
    ltdmabuf[3] = 0;  /* address */
    ltdmabuf[4] = 0;
    ltdmabuf[5] = 1;  /* read 0x0100 bytes */
