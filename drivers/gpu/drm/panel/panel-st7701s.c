...
struct w280bf036i {
	...
	struct w280bf036i_panel_desc *desc;
...
static int w280bf036i_get_modes(struct drm_panel *panel)
{
	...
	struct drm_display_mode *desc_mode = w280bf036i->desc->mode;
...
static struct drm_display_mode w280bf036i_mode = {
	.clock = 25000,

	.hdisplay = 480,
	.hsync_start = 480 + /* HFP */ 8,
	.hsync_end = 480 + 8 + /* HSync */ 2,
	.htotal = 480 + 8 + 2 + /* HBP */ 12,

	.vdisplay = 640,
	.vsync_start = 640 + /* VFP */ 8,
	.vsync_end = 640 + 8 + /* VSync */ 2,
	.vtotal = 640 + 8 + 2 + /* VBP */ 12,
...
static struct w280bf036i_panel_desc w280bf036i_desc = {
...
static int w280bf036i_dsi_probe(struct mipi_dsi_device *dsi)
{
	...
	struct w280bf036i_panel_desc *desc = NULL;
	struct w280bf036i *w280bf036i =
	        devm_kzalloc(&dsi->dev, sizeof(*w280bf036i), GFP_KERNEL);
	if (!w280bf036i)
		return -ENOMEM;

	desc = (struct w280bf036i_panel_desc *)of_device_get_match_data(&dsi->dev);
	...
	if(backlight){
		w280bf036i->backlight = of_find_backlight_by_node(backlight);
		of_node_put(backlight);
		if(!w280bf036i->backlight)
			printk("panel without backlight\n");
	}

	drm_panel_init(&w280bf036i->panel);
	w280bf036i->panel.funcs = &w280bf036i_funcs;
	w280bf036i->panel.dev = dev;
...
static int w280bf036i_dsi_remove(struct mipi_dsi_device *dsi)
{
	...
	if(w280bf036i->backlight)
		put_device(&w280bf036i->backlight->dev);
