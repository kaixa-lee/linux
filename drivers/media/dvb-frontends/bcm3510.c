...
static int bcm3510_bert_reset(struct bcm3510_state *st)
{
    ...
    b.BERCTL_fa.RESYNC = 0;
    bcm3510_writeB(st,0xfa,b);
    b.BERCTL_fa.CNTCTL = 1;
    b.BERCTL_fa.BITCNT = 1;
    bcm3510_writeB(st,0xfa,b);
                             ...
                             u16 len)
{
    ...
    vH.MADRH_a9 = addr >> 8;
    vL.MADRL_aa = addr;
    if ((ret = bcm3510_writeB(st,0xa9,vH)) < 0) {
        return ret;
    }
    if ((ret = bcm3510_writeB(st,0xaa,vL)) < 0) {
        return ret;
    }
