...
static int pvr2_ctrl_range_check(struct pvr2_ctrl *cptr,int val)
{
    if (cptr->info->check_value) {
        if (!cptr->info->check_value(cptr,val)) {
            return -ERANGE;
        }
...
int pvr2_ctrl_has_custom_symbols(struct pvr2_ctrl *cptr)
{
    ...
    if (!cptr->info->val_to_sym) {
        return 0;
    }
    if (!cptr->info->sym_to_val) {
        return 0;
    }
    return !0;
}
                           ...
                           int *maskptr,int *valptr)
{
    ...
    *valptr = 0;
    ...
    cnt = 0;
    while ((cnt < len) && ((ptr[cnt] <= 32) || (ptr[cnt] >= 127))) {
        cnt++;
    }
    len -= cnt;
    ptr += cnt;
