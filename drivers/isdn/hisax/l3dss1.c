                      ...
                      int cr, u_char *p)
{
    ...
    if (pc) {
        ...
        return;    /* neither pc nor st specified */
    }
...
static void l3dss1_redir_req(struct l3_process *pc, u_char pr, void *arg)
{
    ...
    for (subp = pc->chan->setup.phone; (*subp) && (*subp != '.'); subp++) {
        len_phone++;    /* len of phone number */
    }
    if (*subp++ == '.') {
        len_sub = strlen(subp) + 2;    /* length including info subaddress element */
    }
