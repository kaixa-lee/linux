...
{
    writeb(value, cs->hw.isurf.isac + offset);
    mb();
}
...
WriteISACfifo(struct IsdnCardState *cs, u_char *data, int size)
{
    register int i;
    for (i = 0; i < size; i++) {
        writeb(data[i], cs->hw.isurf.isac);
        mb();
    }
}
...
{
    writeb(value, cs->hw.isurf.isar + offset);
    mb();
}
...
Start_ISAC:
    ...
    mb();
    writeb(0, cs->hw.isurf.isac + ISAC_MASK);
    mb();
    writeb(ISAR_IRQMSK, cs->hw.isurf.isar + ISAR_IRQBIT);
    mb();
