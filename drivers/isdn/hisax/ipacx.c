...
cic_int(struct IsdnCardState *cs)
{
    ...
    if (cs->debug & L1_DEB_ISAC) {
        debugl1(cs, "cic_int(event=%#x)", event);
    }
//#########################################
...
dch_l2l1(struct PStack *st, int pr, void *arg)
{
    ...
    default:
        if (cs->debug & L1_DEB_WARN) {
            debugl1(cs, "dch_l2l1 unknown %04x", pr);
        }
        break;
    }
}
...
bch_empty_fifo(struct BCState *bcs, int count)
{
    ...
    cnt = count;
    while (cnt--) {
        *ptr++ = cs->BC_Read_Reg(cs, hscx, IPACX_RFIFOB);
    }
    cs->BC_Write_Reg(cs, hscx, IPACX_CMDRB, 0x80);  // RMC
...
bch_setstack(struct PStack *st, struct BCState *bcs)
{
    ...
    if (bch_open_state(st->l1.hardware, bcs)) {
        return (-1);
    }
    st->l1.bcs = bcs;
    st->l2.l2l1 = bch_l2l1;
...
//#################################################
        if (ista & 0x80) {
            bch_int(cs, 0);    // B channel interrupts
        }
        if (ista & 0x40) {
            bch_int(cs, 1);
        }
        ...
        if (ista & 0x01) {
            dch_int(cs);    // D channel
        }
        if (ista & 0x10) {
            cic_int(cs);    // Layer 1 state
        }
    }
}
...
clear_pending_ints(struct IsdnCardState *cs)
{
    ...
    if (ista & 0x80) {
        cs->BC_Read_Reg(cs, 0, IPACX_ISTAB);
    }
    if (ista & 0x40) {
        cs->BC_Read_Reg(cs, 1, IPACX_ISTAB);
    }
    if (ista & 0x10) {
        cs->readisac(cs, IPACX_CIR0);
    }
    if (ista & 0x01) {
        cs->readisac(cs, IPACX_ISTAD);
    }
}
