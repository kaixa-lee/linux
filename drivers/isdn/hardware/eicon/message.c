...
word api_put(APPL *appl, CAPI_MSG *msg)
{
    ...
    for (j = 0; j < MAX_MSG_PARMS + 1; j++) {
        msg_parms[j].length = 0;
    }
        ...
        for (i = 0; i < max_adapter; i++) {
            if (adapter[i].request) {
                for (j = 0; j < adapter[i].max_plci; j++) {
                    if (adapter[i].plci[j].Sig.Id) {
                        plci_remove(&adapter[i].plci[j]);
                    }
                }
            }
        }
                        ...
                        PLCI *plci, APPL *appl, API_PARSE *parms)
{
    ...
    if (a) {
        ...
        if ((i = get_plci(a))) {
                    ...
                    if (ai_parms[0].length) {
                        ...
                        if (ch == 3 && ai_parms[0].length >= 7 && ai_parms[0].length <= 36) {
                            ...
                            if (!Info) {
                                ...
                                if (!dir) {
                                    plci->call_dir |= CALL_DIR_FORCE_OUTG_NL;    /* dir 0=DTE, 1=DCE */
                                }
                            }
                        }
                    }
            ...
            if (ch == 1 && LinkLayer != 3 && LinkLayer != 12) {
                noCh = true;
            }
            if ((ch == 0 || ch == 2 || noCh || ch == 3 || ch == 4) && !Info) {
                ...
                if (noCh) {
                    Info = add_b1(plci, &parms[5], 2, 0);    /* no resource    */
                } else {
                    Info = add_b1(plci, &parms[5], ch, 0);
                }
            ...
            if (!Info) {
                if (ch == 0 || ch == 2 || ch == 3 || noCh || ch == 4) {
                    ...
                    if (ch == 4) {
                        add_p(plci, CHI, p_chi);
                    }
                    add_s(plci, CPN, &parms[1]);
                    add_s(plci, DSA, &parms[3]);
                    if (noCh) {
                        add_p(plci, ESC, "\x02\x18\xfd");    /* D-channel, no B-L3 */
                    }
                    add_ai(plci, &parms[9]);
                    if (!dir) {
    ...
    if (a) {
        ...
        if (Id&EXT_CONTROLLER && GET_DWORD(parms[1].info)) {
            if (a->profile.Global_Options & ON_BOARD_CODEC) {
                ...
                Info = 0x2002;    /* wrong controller, codec not supported */
            }
        } else {            /* clear listen */
                              ...
                              PLCI *plci, APPL *appl, API_PARSE *parms)
{
    ...
    if (plci && ncci) {
        ...
        if (i < MAX_CHANNELS_PER_PLCI) {
            ...
            for (; i < MAX_CHANNELS_PER_PLCI - 1; i++) {
                plci->inc_dis_ncci_table[i] = plci->inc_dis_ncci_table[i + 1];
            }
                       ...
                       byte nl_rc)
{
    ...
    if (appl) {
        ...
        if (plci->tel && plci->SuppState != CALL_HELD) {
            Id |= EXT_CONTROLLER;
        }
        Number = plci->number;
        ...
        if (plci->command) {
            ...
            case MWI_DEACTIVATE_REQ_PEND:
                ...
                if (plci->internal_command == MWI_ACTIVATE_REQ_PEND) {
                    ...
                    PUT_WORD(&SSparms[1], S_MWI_DEACTIVATE);
                }
            ...
            case CONF_REATTACH_REQ_PEND:
                ...
                if ((plci->internal_command == CONF_ADD_REQ_PEND) && (!plci->relatedPTYPLCI)) {
                    break;
                }
                rplci = plci;
                rId = Id;
...
#define MAXPARMSIDS 31
    ...
    case S_SERVICE_REJ:
        ...
        case DEACTIVATION_MWI:
            if (pty_cai[5] == ACTIVATION_MWI) {
                ...
                PUT_WORD(&SS_Ind[1], S_MWI_DEACTIVATE);
            }
            ...
            if (pty_cai[0] >= 0x12) {
                ...
                if (plci->appl && (a->Notification_Mask[plci->appl->Id - 1] & SMASK_MWI)) {
                    if (plci->internal_command == GET_MWI_STATE) { /* result on Message Waiting Listen */
                        ...
                        sendf(plci->appl, _FACILITY_I, Id, 0, "wS", 3, &pty_cai[2]);
                    }
                    pty_cai[0] = 0;
                } else {
                    for (i = 0; i < max_appl; i++) {
    ...
    case HANGUP:
        plci->hangup_flow_ctrl_timer = 0;
        if (plci->manufacturer && plci->State == LOCAL_CONNECT) {
            break;
        }
        cau = parms[7];
        if (cau) {
    ...
    for (i = 0; i < MAXPARMSIDS; i++) {
        ...
        if (ie[0]) {
            ...
            case 17: /* if no 1tr6 cause, send full cause, else esc_cause */
                ...
                if (!cause[2] || cause[2] < 0x80) {
                    break;    /* eg. layer 1 error */
                }
                Info_Number = 0x0008;
                Info_Mask = 0x01;
                if (cause[2] != ie[2]) {
...
static void nl_ind(PLCI *plci)
{
    ...
    if ((plci->NL.Ind & 0x0f) == N_CONNECT ||
        ...
        case  2: /*ISO8202*/
        case  3: /*X25 DCE*/
            for (i = 0; i < plci->NL.RLength; i++) {
                plci->ncpi_buffer[4 + i] = plci->NL.RBuffer->P[i];
            }
    ...
    case N_DATA:
        ...
        for (i = 0; i < APPLptr->MaxBuffer; i++) {
            if (NCCIcode == APPLptr->DataNCCI[i]) {
                count++;
            }
            if (!APPLptr->DataNCCI[i] && Num == 0xffff) {
                Num = i;
            }
        }
...
static word get_plci(DIVA_CAPI_ADAPTER *a)
{
    ...
    plci->ncci_ring_list = 0;
    for (j = 0; j < MAX_CHANNELS_PER_PLCI; j++) {
        plci->inc_dis_ncci_table[j] = 0;
    }
    clear_c_ind_mask(plci);
...
{
    if (p) {
        add_ie(plci, code, p->info, (word)p->length);
    }
}
...
static byte getChannel(API_PARSE *p)
{
    ...
    if (p) {
        for (i = 2; i < (byte)p->length; i += p->info[i] + 2) {
            if (p->info[i] == 2) {
                if (p->info[i - 1] == ESC && p->info[i + 1] == CHI) {
                    return (p->info[i + 2]);
                }
            }
        }
...
static void add_ie(PLCI *plci, byte code, byte *p, word p_length)
{
    ...
    if (p) {
        ...
        for (i = 0; i < p_length; i++) {
            plci->RBuffer[plci->req_in++] = p[1 + i];
        }
    }
        ...
        if ((llc[1] == V120_L2) || (llc[1] == V120_V42BIS)) {
            ...
            if (b2_config->length) {
                ...
                if (llc[1] == 6) {
                    ...
                    if (b2_config->length >= 4) {
                        dlc[4] = b2_config->info[4];    /* Window */
                    } else {
                        dlc[4] = 1;
                    }
...
static word add_modem_b23(PLCI *plci, API_PARSE *bp_parms)
{
    ...
    for (i = 0; i < 2; i++) {
        mdm_config[i].length = 0;
    }
    for (i = 0; i < sizeof(dlc); i++) {
        dlc[i] = 0;
    }
...
static void sig_req(PLCI *plci, byte req, byte Id)
{
    ...
    if (plci->adapter->adapter_disabled) {
        return;
    }
...
static byte ie_compare(byte *ie1, byte *ie2)
{
    ...
    for (i = 0; i < (word)(ie1[0] + 1); i++) if (ie1[i] != ie2[i]) {
            return false;
        }
    return true;
}
                            ...
                            byte hook_listen)
{
    ...
    if (a->profile.Global_Options & HANDSET) {
        ...
        if ((j = get_plci(a))) {
            ...
            if (hook_listen) {
                splci->State = ADVANCED_VOICE_SIG;
            } else {
...
static byte chi_to_channel(byte *chi, dword *pchannelmap)
{
    ...
    if (chi[1] & 0x20) {
        if (chi[0] == 1 && chi[1] == 0xac) {
            return 0xfd;    /* exclusive d-channel */
        }
        for (i = 1; i < chi[0] && !(chi[i] & 0x80); i++);
        if (i == chi[0] || !(chi[i] & 0x80)) {
            return 0xfe;
        }
        if ((chi[1] | 0xc8) != 0xe9) {
            return 0xfe;
