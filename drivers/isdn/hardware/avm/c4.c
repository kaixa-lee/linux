...
static int c4_detect(avmcard *card)
{
    ...
    if (c4inmeml(card->mbase + MAILBOX_0) != 0x55aa55aa) {
        return 3;
    }

    c4outmeml(card->mbase + MAILBOX_0, 0xaa55aa55);
    if (c4inmeml(card->mbase + MAILBOX_0) != 0xaa55aa55) {
        return 4;
    }

    if (c4_poke(card, DC21285_ARMCSR_BASE + DBELL_SA_MASK, 0)) {
        return 5;
    }
    if (c4_poke(card, DC21285_ARMCSR_BASE + DBELL_PCI_MASK, 0)) {
        return 6;
    }
    ...
    if (c4_poke(card, DC21285_ARMCSR_BASE + DRAM_TIMING, 0)) {
        return 9;
    }

    mdelay(1);

    if (c4_peek(card, DC21285_DRAM_A0MR, &dummy)) {
        return 10;
    }
    if (c4_peek(card, DC21285_DRAM_A1MR, &dummy)) {
        return 11;
    }
    if (c4_peek(card, DC21285_DRAM_A2MR, &dummy)) {
        return 12;
    }
    if (c4_peek(card, DC21285_DRAM_A3MR, &dummy)) {
        return 13;
    }

    if (c4_poke(card, DC21285_DRAM_A0MR + CAS_OFFSET, 0)) {
        return 14;
    }
    if (c4_poke(card, DC21285_DRAM_A1MR + CAS_OFFSET, 0)) {
        return 15;
    }
    if (c4_poke(card, DC21285_DRAM_A2MR + CAS_OFFSET, 0)) {
        return 16;
    }
    if (c4_poke(card, DC21285_DRAM_A3MR + CAS_OFFSET, 0)) {
        return 17;
    }
                             ...
                             capi_register_params *rp)
{
    ...
    if (ctrl->cnr == card->cardnr) {
        ...
        if (want > 0) {
            ...
            nconn = ctrl->profile.nbchannel * 4 * -want;
        }
        if (nconn == 0) {
            nconn = ctrl->profile.nbchannel * 4;
        }
