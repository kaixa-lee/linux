               ...
               u_char service, char *fwd_nr, ulong *procid)
{
    ...
    if (strchr(msn, '.')) {
        return (-EINVAL);    /* subaddress not allowed in msn */
    }
    if ((proc & 0x7F) > 2) {
        return (-EINVAL);
    }
    ...
    *p++ = service; /* service to handle */
    ...
    if (mode == 1) {
        if (!*fwd_nr) {
            return (-EINVAL);    /* destination missing */
        }
        if (strchr(fwd_nr, '.')) {
            return (-EINVAL);    /* subaddress not allowed */
        }
...
int deflect_extern_action(u_char cmd, ulong callid, char *to_nr)
{
    ...
    while (cs) {
        if (cs->divert_id == callid) {
            break;    /* found */
        }
        cs = cs->next;
    } /* search entry */
    if (!cs) {
        return (-EINVAL);    /* invalid callid */
    }

    ic.driver = cs->ics.driver;
    ic.arg = cs->ics.arg;
    i = -EINVAL;
    if (cs->akt_state == DEFLECT_AUTODEL) {
        return (i);    /* no valid call */
    }
    switch (cmd & 0x7F) {
    case 0: /* hangup */
        del_timer(&cs->timer);
...
static int isdn_divert_icall(isdn_ctrl *ic)
{
    ...
    for (dv = table_head; dv; dv = dv->next) {
        ...
        case DEFLECT_REJECT:
            ...
            if (!cs->ics.parm.setup.eazmsn[0]) {
                strcpy(cs->ics.parm.setup.eazmsn, "0");
            }
...
static int interrogate_success(isdn_ctrl *ic, struct call_struc *cs)
{
    ...
    if (restlen < 2) {
        return (-100);    /* frame too short */
    }
    if (*src++ != 0x30) {
        return (-101);
    }
    if ((n = *src++) > 0x81) {
        return (-102);    /* invalid length field */
    }
    ...
    if (n == 0x80) {
        ...
        if ((*(src + restlen - 1)) || (*(src + restlen - 2))) {
            return (-104);
        }
        restlen -= 2;
    } else if (n == 0x81) {
    ...
    if (restlen < 3) {
        return (-107);    /* no procedure */
    }
    if ((*src++ != 2) || (*src++ != 1) || (*src++ != 0x0B)) {
        return (-108);
    }
    restlen -= 3;
    if (restlen < 2) {
        return (-109);    /* list missing */
    }
    if (*src == 0x31) {
        src++;
        if ((n = *src++) > 0x81) {
            return (-110);    /* invalid length field */
        }
        restlen -= 2; /* remaining bytes */
        if (n == 0x80) {
            if (restlen < 2) {
    ...
    while (restlen >= 2) {
        ...
        p = src; /* one entry */
        src += n;
        if (!(n1 = put_address(stp, p, n & 0xFF))) {
            continue;
        }
        stp += strlen(stp);
        p += n1;
