    ...
    */
    ...
    if (tp->usrflag & UF_TRACE) {
        ...
        if (cp->host_status==HS_COMPLETE) {
            ...
            printk ("  HOSTERROR: %x", cp->host_status);
        }
        printk ("\n");
    }
...
static void ncr_getsync(struct ncb *np, u_char sfac, u_char *fakp, u_char *scntl3p)
{
    ...
    */
    ...
    if (fak < 4) {
        fak = 4;    /* Should never happen, too bad ... */
    }
...
static void ncr_setup_tags (struct ncb *np, struct scsi_device *sdev)
{
    ...
    */
    ...
    if (maxdepth > lp->maxnxs)	{
        maxdepth    = lp->maxnxs;
    }
    if (lp->maxtags > maxdepth)	{
        lp->maxtags = maxdepth;
    }
    if (lp->numtags > maxdepth)	{
        lp->numtags = maxdepth;
    }
