...
static int change_diag(struct atm_dev *dev,void __user *arg,int set)
{
    ...
    if (get_user(todo,(int __user *)arg)) {
        return -EFAULT;
    }
...
static int get_diag(struct atm_dev *dev,void __user *arg)
{
    ...
    set = 0;
    if (GET(TSOP_DIAG) & SUNI_TSOP_DIAG_DBIP8) {
        set |= SONET_INS_SBIP;
    }
    if (GET(TLOP_DIAG) & SUNI_TLOP_DIAG_DBIP) {
        set |= SONET_INS_LBIP;
    }
    if (GET(TPOP_CD) & SUNI_TPOP_DIAG_DB3) {
        set |= SONET_INS_PBIP;
    }
    /* SONET_INS_FRAME is one-shot only */
    if (GET(TSOP_CTRL) & SUNI_TSOP_CTRL_LAIS) {
        set |= SONET_INS_LAIS;
    }
    if (GET(TPOP_CD) & SUNI_TPOP_DIAG_PAIS) {
        set |= SONET_INS_PAIS;
    }
    if (GET(TSOP_DIAG) & SUNI_TSOP_DIAG_DLOS) {
        set |= SONET_INS_LOS;
    }
    if (GET(TACP_CS) & SUNI_TACP_CS_DHCS) {
        set |= SONET_INS_HCS;
    }
