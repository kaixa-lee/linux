...
err_out:
    ...
    */
    ...
    static int open_tx(struct atm_vcc *vcc) {
        ...
        if (ia_vcc->pcr > (iadev->LineRate / 6) ) {
            ia_vcc->ltimeout = HZ / 10;
        } else if (ia_vcc->pcr > (iadev->LineRate / 130)) {
            ia_vcc->ltimeout = HZ;
        } else if (ia_vcc->pcr <= 170) {
            ia_vcc->ltimeout = 16 * HZ;
        } else {
            ia_vcc->ltimeout = 2700 * HZ  / ia_vcc->pcr;
        }
        ...
        case MEMDUMP: {
            ...
            case MEMDUMP_SEGREG:
                ...
                for(i=0; i<0x80; i+=2, tmps++)
                    if(put_user((u16)(readl(iadev->seg_reg+i) & 0xffff), tmps)) {
                        return -EFAULT;
                    }
                ia_cmds.status = 0;
                ia_cmds.len = 0x80;
            ...
            case MEMDUMP_REASSREG:
                ...
                tmps = (u16 __user *)ia_cmds.buf;
                for(i=0; i<0x80; i+=2, tmps++)
                    if(put_user((u16)(readl(iadev->reass_reg+i) & 0xffff), tmps)) {
                        return -EFAULT;
                    }
                ia_cmds.status = 0;
                ia_cmds.len = 0x80;
