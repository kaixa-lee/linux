...
static void refill_pool(struct atm_dev *dev,int pool)
{
    ...
    if (free >= zatm_dev->pool_info[pool].low_water) {
        return;
    }
...
static int alloc_shaper(struct atm_dev *dev,int *pcr,int min,int max,int ubr)
{
    ...
    if (ubr) {
        ...
        *pcr = i*ATM_OC3_PCR/m;
        ...
        if ((min && *pcr < min) || (max && *pcr > max)) {
            return -EINVAL;
        }
        if (zatm_dev->tx_bw < *pcr) {
            return -EAGAIN;
        }
        zatm_dev->tx_bw -= *pcr;
    }
...
static int start_tx(struct atm_dev *dev)
{
    ...
    for (i = 0; i < NR_SHAPERS; i++) {
        zpokel(zatm_dev,0,uPD98401_PS(i));
    }
    return 0;
}
                           ...
                           unsigned short cmd)
{
    ...
    *byte = 0;
    for (i = 8; i; i--) {
        ...
        *byte <<= 1;
        if (eprom_get(zatm_dev,cmd) & ZEPROM_DO) {
            *byte |= 1;
        }
        eprom_set(zatm_dev,ZEPROM_CS,cmd);
    }
...
{
    if (eprom_try_esi(dev,ZEPROM_V1_REG,ZEPROM_V1_ESI_OFF,1)) {
        return;
    }
...
static int zatm_init(struct atm_dev *dev)
{
    ...
    for (i = last-RAM_INCREMENT; i >= 0; i -= RAM_INCREMENT) {
        ...
        if (zpeekl(zatm_dev,i) != 0x55555555) {
            last = i;
        } else {
            zpokel(zatm_dev,0xAAAAAAAA,i);
            if (zpeekl(zatm_dev,i) != 0xAAAAAAAA) {
                last = i;
            } else {
                zpokel(zatm_dev,i,i);
            }
