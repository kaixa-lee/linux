...
static int nv3_iterate(nv3_fifo_info *res_info, nv3_sim_state * state, nv3_arb_info *ainfo)
{
    ...
    gmisses = 2;
    vmisses = 2;
    if (ainfo->gburst_size == 128) {
        max_gfsize = GFIFO_SIZE_128;
    } else {
    ...
    while (1) {
        if (ainfo->vid_en) {
            if (ainfo->wcvocc > ainfo->vocc) {
                ainfo->wcvocc = ainfo->vocc;
            }
            if (ainfo->wcvlwm > vlwm) {
                ainfo->wcvlwm = vlwm ;
            }
        ...
        if (state->enable_mp) {
            if (ainfo->wcmocc > ainfo->mocc) {
                ainfo->wcmocc = ainfo->mocc;
            }
        }
        if (ainfo->gr_en) {
            if (ainfo->wcglwm > glwm) {
                ainfo->wcglwm = glwm ;
            }
            if (ainfo->wcgocc > ainfo->gocc) {
                ainfo->wcgocc = ainfo->gocc;
            }
...
)
{
    ...
    case NV_ARCH_30:
        ...
        if(chip->flatPanel) {
            ...
            } else if((chip->Chipset & 0x0ff0) >= 0x0170) {
