...
void print_pte (pte_t pte)
{
    ...
    if (val & SUN3_PAGE_VALID)	{
        printk (" valid");
    }
    if (val & SUN3_PAGE_WRITEABLE)	{
        printk (" write");
    }
    if (val & SUN3_PAGE_SYSTEM)	{
        printk (" sys");
    }
    if (val & SUN3_PAGE_NOCACHE)	{
        printk (" nocache");
    }
    if (val & SUN3_PAGE_ACCESSED)	{
        printk (" accessed");
    }
    if (val & SUN3_PAGE_MODIFIED)	{
        printk (" modified");
    }
    switch (val & SUN3_PAGE_TYPE_MASK) {
    case SUN3_PAGE_TYPE_MEMORY:
        printk (" memory");
        break;
    case SUN3_PAGE_TYPE_IO:
        printk (" io");
        break;
    case SUN3_PAGE_TYPE_VME16:
        printk (" vme16");
        break;
    case SUN3_PAGE_TYPE_VME32:
        printk (" vme32");
        break;
    }
    printk ("]\n");
