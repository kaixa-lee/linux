...
pfm_buf_fmt_exit(pfm_buffer_fmt_t *fmt, struct task_struct *task, void *buf, struct pt_regs *regs)
{
    int ret = 0;
    if (fmt->fmt_exit) {
        ret = (*fmt->fmt_exit)(task, buf, regs);
    }
    return ret;
}
...
pfm_buf_fmt_getsize(pfm_buffer_fmt_t *fmt, struct task_struct *task, unsigned int flags, int cpu, void *arg, unsigned long *size)
{
    int ret = 0;
    if (fmt->fmt_getsize) {
        ret = (*fmt->fmt_getsize)(task, flags, cpu, arg, size);
    }
    return ret;
}
                     ...
                     int cpu, void *arg)
{
    int ret = 0;
    if (fmt->fmt_validate) {
        ret = (*fmt->fmt_validate)(task, flags, cpu, arg);
    }
    return ret;
}
                 ...
                 int cpu, void *arg)
{
    int ret = 0;
    if (fmt->fmt_init) {
        ret = (*fmt->fmt_init)(task, buf, flags, cpu, arg);
    }
    return ret;
}
...
pfm_buf_fmt_restart(pfm_buffer_fmt_t *fmt, struct task_struct *task, pfm_ovfl_ctrl_t *ctrl, void *buf, struct pt_regs *regs)
{
    int ret = 0;
    if (fmt->fmt_restart) {
        ret = (*fmt->fmt_restart)(task, ctrl, buf, regs);
    }
    return ret;
}
...
{
    int ret = 0;
    if (fmt->fmt_restart_active) {
        ret = (*fmt->fmt_restart_active)(task, ctrl, buf, regs);
    }
    return ret;
}
...
pfm_read(struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
    ...
    ret = -EFAULT;
    if(copy_to_user(buf, msg, sizeof(pfm_msg_t)) == 0) {
        ret = sizeof(pfm_msg_t);
    }
...
pfm_flush(struct file *filp, fl_owner_t id)
{
     ...
     */
    if (smpl_buf_vaddr) {
        pfm_remove_smpl_mapping(smpl_buf_vaddr, smpl_buf_size);
    }
...
doit:
    ...
    if (smpl_buf_addr) {
        pfm_exit_smpl_buffer(ctx->ctx_buf_fmt);
    }
     ...
     */
    if (smpl_buf_addr) {
        pfm_rvfree(smpl_buf_addr, smpl_buf_size);
    }
...
#define PFM_CTXARG_BUF_ARG(a)	(pfm_buffer_fmt_t *)(a+1)
     ...
     */
    if (fmt->fmt_arg_size) {
        fmt_arg = PFM_CTXARG_BUF_ARG(arg);
    }
...
pfm_write_pmds(pfm_context_t *ctx, void *arg, int count, struct pt_regs *regs)
{
    ...
    if (unlikely(state == PFM_CTX_ZOMBIE)) {
        return -EINVAL;
    }
...
pfm_read_pmds(pfm_context_t *ctx, void *arg, int count, struct pt_regs *regs)
{
    ...
    for (i = 0; i < count; i++, req++) {
         ...
         */
        if (unlikely(!CTX_IS_USED_PMD(ctx, cnum))) {
            goto error;
        }
...
pfm_mod_write_pmcs(struct task_struct *task, void *req, unsigned int nreq, struct pt_regs *regs)
{
     ...
     */
    if (task != current && ctx->ctx_fl_system == 0) {
        return -EBUSY;
    }
...
pfm_use_debug_registers(struct task_struct *task)
{
     ...
     */
    if (ctx && ctx->ctx_fl_using_dbreg == 1) {
        return -1;
    }
...
pfm_restart(pfm_context_t *ctx, void *arg, int count, struct pt_regs *regs)
{
    ...
    case PFM_CTX_MASKED:
        break;
    case PFM_CTX_LOADED:
        if (CTX_HAS_SMPL(ctx) && fmt->fmt_restart_active) {
            break;
        }
    /* fall through */
...
error_unres:
    if (ret) {
        pfm_unreserve_session(ctx, ctx->ctx_fl_system, the_cpu);
    }
error:
    /*
...
restart_args:
    ...
    if (unlikely((cmd_flags & PFM_CMD_FD) == 0)) {
        goto skip_fd;
    }
...
abort_locked:
    ...
    if (call_made && PFM_CMD_RW_ARG(cmd) && copy_to_user(arg, args_k, base_sz*count)) {
        ret = -EFAULT;
    }
                                 ...
                                 unsigned long pmc0, struct pt_regs *regs)
{
     ...
     */
    if (must_notify) {
        pfm_ovfl_notify_user(ctx, ovfl_notify);
    }
...
pfm_load_regs (struct task_struct *task)
{
     ...
     * possible on unload
     */
    if (unlikely((task->thread.flags & IA64_THREAD_PM_VALID) == 0)) {
        return;
    }
...
pfm_init(void)
{
    ...
    for(i=0; i < NR_CPUS; i++) {
        pfm_stats[i].pfm_ovfl_intr_cycles_min = ~0UL;
    }
