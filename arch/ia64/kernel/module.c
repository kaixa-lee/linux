          ...
          Elf64_Shdr *sec, void *location)
{
    ...
    case RV_SECREL:
        ...
        break;
    case RV_LTREL_FPTR:
        val = get_ltoff(mod, get_fdesc(mod, val, &ok), &ok);
        break;
    ...
    switch (format) {
    case RF_INSN21B:
        ok = apply_imm21b(mod, location, (int64_t) val / 16);
        break;
    case RF_INSN22:
        ok = apply_imm22(mod, location, val);
        break;
    case RF_INSN64:
        ok = apply_imm64(mod, location, val);
        break;
    case RF_INSN60:
        ok = apply_imm60(mod, location, (int64_t) val / 16);
        break;
    case RF_32LSB:
        put_unaligned(val, (uint32_t *) location);
        break;
    case RF_64LSB:
        put_unaligned(val, (uint64_t *) location);
        break;
