...
static void __init psurge_quad_init(void)
{
    ...
    if (ppc_md.progress) {
        ppc_md.progress("psurge_quad_init", 0x351);
    }
...
static void __init smp_psurge_probe(void)
{
    ...
    if (ppc_md.progress) {
        ppc_md.progress("smp_psurge_probe - done", 0x352);
    }
}
...
static int __init smp_psurge_kick_cpu(int nr)
{
    ...
    if (ppc_md.progress) {
        ppc_md.progress("smp_psurge_kick_cpu", 0x353);
    }
...
stuck:
    ...
    if (ppc_md.progress) {
        ppc_md.progress("smp_psurge_kick_cpu - done", 0x354);
    }
...
static void __init smp_core99_probe(void)
{
    ...
    if (ppc_md.progress) {
        ppc_md.progress("smp_core99_probe", 0x345);
    }
...
static int smp_core99_kick_cpu(int nr)
{
    ...
    *vector = save_vector;
    ...
    local_irq_restore(flags);
    if (ppc_md.progress) {
        ppc_md.progress("smp_core99_kick_cpu done", 0x347);
    }
