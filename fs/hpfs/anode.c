                        ...
                        struct buffer_head *bh)
{
    ...
    int c1, c2 = 0;
go_down:
    if (hpfs_sb(s)->sb_chk) if (hpfs_stop_cycles(s, a, &c1, &c2, "hpfs_bplus_lookup")) {
            return -1;
        }
...
go_down:
    ...
    if (bp_internal(btree)) {
        ...
        brelse(bh);
        if (hpfs_sb(s)->sb_chk)
            if (hpfs_stop_cycles(s, a, &c1, &c2, "hpfs_add_sector_to_btree #1")) {
                return -1;
            }
        if (!(anode = hpfs_map_anode(s, a, &bh))) {
            return -1;
    ...
    while (up != (anode_secno)-1) {
        ...
        if (hpfs_sb(s)->sb_chk)
            if (hpfs_stop_cycles(s, up, &c1, &c2, "hpfs_add_sector_to_btree #2")) {
                return -1;
            }
        if (up != node || !fnod) {
            if (!(anode = hpfs_map_anode(s, up, &bh))) {
                return -1;
            }
            btree = &anode->btree;
        } else {
            if (!(fnode = hpfs_map_fnode(s, up, &bh))) {
                return -1;
            }
            btree = &fnode->btree;
        }
...
go_down:
    ...
    while (bp_internal(btree1)) {
        ...
        if (!(anode = hpfs_map_anode(s, ano, &bh))) {
            return;
        }
        btree1 = &anode->btree;
        level++;
...
go_up:
    ...
    brelse(bh);
    if (hpfs_sb(s)->sb_chk)
        if (hpfs_stop_cycles(s, ano, &c1, &c2, "hpfs_remove_btree #2")) {
            return;
        }
                 ...
                 unsigned len, char *buf)
{
    ...
    while (len) {
        ...
        if (hpfs_sb(s)->sb_chk) if (hpfs_chk_sectors(s, sec, 1, "ea #1")) {
                return -1;
            }
                  ...
                  unsigned len, const char *buf)
{
    ...
    while (len) {
        ...
        if (hpfs_sb(s)->sb_chk) if (hpfs_chk_sectors(s, sec, 1, "ea #2")) {
                return -1;
            }
        if (!(data = hpfs_map_sector(s, sec, &bh, (len - 1) >> 9))) {
            return -1;
...
void hpfs_truncate_btree(struct super_block *s, secno f, int fno, unsigned secs)
{
    ...
    int c1, c2 = 0;
    if (fno) {
        if (!(fnode = hpfs_map_fnode(s, f, &bh))) {
            return;
        }
        btree = &fnode->btree;
    } else {
        if (!(anode = hpfs_map_anode(s, f, &bh))) {
            return;
        }
        btree = &anode->btree;
    }
    ...
    while (bp_internal(btree)) {
        ...
        for (i = 0; i < btree->n_used_nodes; i++)
            if (le32_to_cpu(btree->u.internal[i].file_secno) >= secs) {
                goto f;
            }
        brelse(bh);
...
f:
    ...
    nodes = btree->n_used_nodes + btree->n_free_nodes;
    for (i = 0; i < btree->n_used_nodes; i++)
        if (le32_to_cpu(btree->u.external[i].file_secno) + le32_to_cpu(btree->u.external[i].length) >= secs) {
            goto ff;
        }
    brelse(bh);
    return;
...
void hpfs_remove_fnode(struct super_block *s, fnode_secno fno)
{
    ...
    if (!(fnode = hpfs_map_fnode(s, fno, &bh))) {
        return;
    }
    if (!fnode_is_dir(fnode)) {
        hpfs_remove_btree(s, &fnode->btree);
    } else {
        hpfs_remove_dtree(s, le32_to_cpu(fnode->u.external[0].disk_secno));
    }
