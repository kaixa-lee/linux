...
static loff_t get_pos(struct dnode *d, struct hpfs_dirent *fde)
{
    ...
    for (de = dnode_first_de(d); de < de_end; de = de_next_de(de)) {
        if (de == fde) {
            return ((loff_t) le32_to_cpu(d->self) << 4) | (loff_t)i;
        }
        i++;
    }
    pr_info("%s(): not_found\n", __func__);
...
void hpfs_del_pos(struct inode *inode, loff_t *pos)
{
    ...
    for (i = hpfs_inode->i_rddir_off; *i; i++) if (*i == pos) {
            goto fnd;
        }
    goto not_f;
fnd:
                         ...
                         loff_t p1, loff_t p2)
{
    ...
    for (i = hpfs_inode->i_rddir_off; *i; i++) {
        (*f)(*i, p1, p2);
    }
    return;
}
...
down:
    if (hpfs_sb(i->i_sb)->sb_chk)
        if (hpfs_stop_cycles(i->i_sb, dno, &c1, &c2, "hpfs_add_dirent")) {
            return 1;
        }
    if (!(d = hpfs_map_dnode(i->i_sb, dno, &qbh))) {
        return 1;
...
static secno move_to_top(struct inode *i, dnode_secno from, dnode_secno to)
{
    ...
    while (1) {
        ...
        if (!(dnode = hpfs_map_dnode(i->i_sb, dno, &qbh))) {
            return 0;
        }
        if (hpfs_sb(i->i_sb)->sb_chk) {
    ...
    while (!(de = dnode_pre_last_de(dnode))) {
        ...
        if (!(dnode = hpfs_map_dnode(i->i_sb, up, &qbh))) {
            return 0;
        }
        if (dnode->root_dnode) {
...
static void delete_empty_dnode(struct inode *i, dnode_secno dno)
{
    ...
    int c1, c2 = 0;
try_it_again:
    if (hpfs_stop_cycles(i->i_sb, dno, &c1, &c2, "delete_empty_dnode")) {
        return;
    }
    if (!(dnode = hpfs_map_dnode(i->i_sb, dno, &qbh))) {
        return;
    }
    if (le32_to_cpu(dnode->first_free) > 56) {
        goto end;
    }
    if (le32_to_cpu(dnode->first_free) == 52 || le32_to_cpu(dnode->first_free) == 56) {
        ...
        for (de = dnode_first_de(dnode); de < de_end; de = de_next_de(de), p++)
            if (de->down) if (de_down_pointer(de) == dno) {
                    goto fnd;
                }
...
go_down:
    ...
    if (hpfs_sb(s)->sb_chk)
        if (hpfs_stop_cycles(s, dno, &c1, &c2, "hpfs_count_dnodes #1")) {
            return;
        }
    ptr = 0;
go_up:
    ...
    de = dnode_first_de(dnode);
    if (ptr) while(1) {
            if (de->down) if (de_down_pointer(de) == ptr) {
                    goto process_de;
                }
            if (de->last) {
                hpfs_brelse4(&qbh);
...
process_de:
    if (!de->first && !de->last && de->directory && n_subdirs) {
        (*n_subdirs)++;
    }
    if (!de->first && !de->last && n_items) {
        (*n_items)++;
    }
    if ((de = de_next_de(de)) < dnode_end_de(dnode)) {
        goto next_de;
    }
    ptr = dno;
    ...
    hpfs_brelse4(&qbh);
    if (hpfs_sb(s)->sb_chk)
        if (hpfs_stop_cycles(s, ptr, &d1, &d2, "hpfs_count_dnodes #2")) {
            return;
        }
    odno = -1;
    goto go_up;
...
again:
    ...
    if (!(de = map_nth_dirent(s, d, 1, &qbh, NULL))) {
        return dno;
    }
    if (hpfs_sb(s)->sb_chk)
...
again:
    if (hpfs_sb(inode->i_sb)->sb_chk)
        if (hpfs_stop_cycles(inode->i_sb, dno, &c1, &c2, "map_dirent")) {
            return NULL;
        }
    if (!(dnode = hpfs_map_dnode(inode->i_sb, dno, qbh))) {
        return NULL;
    }
...
void hpfs_remove_dtree(struct super_block *s, dnode_secno dno)
{
    ...
    do {
        while (d1) {
            if (!(dnode = hpfs_map_dnode(s, dno = d1, &qbh))) {
                return;
            }
            de = dnode_first_de(dnode);
            if (!de->last) {
...
go_up:
    ...
    if (downd) {
        while (de < de_end) {
            if (de->down) if (de_down_pointer(de) == downd) {
                    goto f;
                }
            de = de_next_de(de);
        }
