...
static int chk_if_allocated(struct super_block *s, secno sec, char *msg)
{
    ...
    __le32 *bmp;
    if (!(bmp = hpfs_map_bitmap(s, sec >> 14, &qbh, "chk"))) {
        goto fail;
    }
...
int hpfs_chk_sectors(struct super_block *s, secno start, int len, char *msg)
{
    ...
    if (hpfs_sb(s)->sb_chk>=2) {
        int i;
        for (i = 0; i < len; i++)
            if (chk_if_allocated(s, start + i, msg)) {
                return 1;
            }
    }
    return 0;
...
static secno alloc_in_bmp(struct super_block *s, secno near, unsigned n, unsigned forward)
{
    ...
    if (bs != ~0x3fff) {
        if (!(bmp = hpfs_map_bitmap(s, near >> 14, &qbh, "aib"))) {
            goto uls;
        }
    } else {
        if (!(bmp = hpfs_map_dnode_bitmap(s, &qbh))) {
    ...
    i = nr;
    do {
        if (!le32_to_cpu(bmp[i])) {
            goto cont;
        }
        if (n + forward >= 0x3f && le32_to_cpu(bmp[i]) != 0xffffffff) {
            goto cont;
        }
        q = i<<5;
        if (i > 0) {
...
secno hpfs_alloc_sector(struct super_block *s, secno near, unsigned n, int forward)
{
    ...
    if (near && near < sbi->sb_fs_size) {
        if ((sec = alloc_in_bmp(s, near, n, f_p ? forward : forward/4))) {
            goto ret;
        }
        near_bmp = near >> 14;
    } else {
    ...
    */
    if (!f_p) if (forward > sbi->sb_max_fwd_alloc) {
            forward = sbi->sb_max_fwd_alloc;
        }
less_fwd:
    for (i = 0; i < n_bmps; i++) {
...
int hpfs_alloc_if_possible(struct super_block *s, secno sec)
{
    struct quad_buffer_head qbh;
    __le32 *bmp;
    if (!(bmp = hpfs_map_bitmap(s, sec >> 14, &qbh, "aip"))) {
        goto end;
    }
...
void hpfs_free_sectors(struct super_block *s, secno sec, unsigned n)
{
    ...
    if (sbi->sb_max_fwd_alloc > 0xffffff) {
...
{
    struct fnode *f;
    if (!(*fno = hpfs_alloc_sector(s, near, 1, FNODE_ALLOC_FWD))) {
        return NULL;
    }
...
{
    struct anode *a;
    if (!(*ano = hpfs_alloc_sector(s, near, 1, ANODE_ALLOC_FWD))) {
        return NULL;
    }
