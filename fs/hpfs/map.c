                        ...
                        struct quad_buffer_head *qbh, char *id)
{
    ...
    if (ret) {
        hpfs_prefetch_bitmap(s, bmp_block + 1);
    }
    return ret;
}
...
unsigned char *hpfs_load_code_page(struct super_block *s, secno cps)
{
    ...
    if (!(cpd = hpfs_map_sector(s, cpds, &bh, 0))) {
        return NULL;
    }
...
{
    struct anode *anode;
    if (hpfs_sb(s)->sb_chk) if (hpfs_chk_sectors(s, ano, 1, "anode")) {
            return NULL;
        }
                             ...
                             struct quad_buffer_head *qbh)
{
    struct dnode *dnode;
    if (hpfs_sb(s)->sb_chk) {
        if (hpfs_chk_sectors(s, secno, 4, "dnode")) {
            return NULL;
        }
        if (secno & 3) {
        ...
        if (hpfs_sb(s)->sb_chk) {
            ...
            for (p = 20; p < le32_to_cpu(dnode->first_free); p += d[p] + (d[p+1] << 8)) {
                ...
                if (((31 + de->namelen + de->down*4 + 3) & ~3) != le16_to_cpu(de->length)) {
                    if (((31 + de->namelen + de->down*4 + 3) & ~3) < le16_to_cpu(de->length) && s->s_flags & MS_RDONLY) {
                        goto ok;
                    }
